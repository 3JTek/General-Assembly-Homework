...
SELECT name, price FROM products
  WHERE price=(SELECT MAX(price) FROM products
    sWHERE price!=(SELECT MAX(price) FROM products));
...
SELECT COUNT(*) FROM wishlists WHERE user_id = (SELECT id FROM users WHERE name LIKE '%James%');
...
-- 18. Create an 18th item on this list.
...
('2019-02-19'),
(SELECT id FROM users WHERE name='Jon Anderson'),
...
UPDATE users SET name= 'Jon Anderson' WHERE id = (SELECT id FROM user WHERE name='Jonathan Anderson');
-- 21. Delete the user with the name "Jon Anderson".
DELETE FROM users WHERE id = (SELECT id FROM user WHERE name='Jon Anderson');
-- 22. Delete the wishlist item for the user you just deleted.
DELETE FROM wishlists
WHERE NOT EXISTS (SELECT NULL FROM users WHERE wishlists.user_id = users.id);
