...
SELECT COUNT(*) FROM wishlists WHERE user_id = (SELECT id FROM users WHERE name LIKE 'Michael%');
...
INSERT INTO wishlists (created_at, user_id, product_id) VALUES (now(), (SELECT id FROM users WHERE name='Jonathan Anderson'), (SELECT id FROM products WHERE name='The Python Programming Language');
