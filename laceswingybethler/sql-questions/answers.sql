...
-- got stuck here! How do i just return the 2nd one, not the top 2?
...
SELECT COUNT(*) FROM users WHERE name='Benjamin';
...
SELECT COUNT(*) FROM wishlists WHERE id=(SELECT id FROM users WHERE name='Beth Swingler');
...
INSERT INTO wishlists(product_id, user_id) VALUES ((SELECT id FROM products WHERE name='The Python Programming Language'), (SELECT id FROM users WHERE name='Jonathan Anderson'));
...
DELETE FROM wishlists WHERE id=(SELECT id FROM wishlists ORDER BY created_at DESC LIMIT 1);
